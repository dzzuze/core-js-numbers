...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const valueD1 = x2 - x1;
  const valueD2 = y2 - y1;
  const valueD1Root = valueD1 ** 2;
  const valueD2Root = valueD2 ** 2;
  const sumD = valueD1Root + valueD2Root;
  const rootD = Math.sqrt(sumD);
  return rootD;
}
...
function getParallelepipedDiagonal(a, b, c) {
  const valueD = Math.sqrt(a ** 2 + b ** 2 + c ** 2);
  return valueD;
}
...
function roundToPowerOfTen(num, pow) {
  const roundTo = Math.round(num / 10 ** pow) * 10 ** pow;
  return roundTo;
}
...
function getSumOfDigits(num) {
  ...
  const valueD = Array.from(numString);
  let valueDSum = 0;
  for (let i = 0; i < valueD.length; i += 1) {
    valueDSum += Number(valueD[i]);
  }
  return valueDSum;
}
